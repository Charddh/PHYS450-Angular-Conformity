...
def fits_to_dataframe(filename):
    """Convert a FITS table HDU to a pandas DataFrame with native-endian columns."""
    with fits.open(filename) as hdul:
        data = hdul[1].data
        df = pd.DataFrame({
            name: data[name].byteswap().view(data[name].dtype.newbyteorder())
            for name in data.dtype.names
        })
    return df

# Load both datasets safely
cluster_df = fits_to_dataframe("catCluster-SPIDERS_RASS_CLUS-v3.0.fits")
df_gal = fits_to_dataframe("GZDR1SFRMASS.fits")

clus_coords = SkyCoord(ra=cluster_df['RA'].values*u.deg,
                      dec=cluster_df['DEC'].values*u.deg)

gal_coords = SkyCoord(ra=df_gal['RA_1'].values*u.deg,
                      dec=df_gal['DEC_1'].values*u.deg)
...
gal_id = nearby_galaxies['SPECOBJID_1'].values
gal_ra = nearby_galaxies['RA_1'].values
gal_dec= nearby_galaxies['DEC_1'].values
gal_z = nearby_galaxies['Z'].values
