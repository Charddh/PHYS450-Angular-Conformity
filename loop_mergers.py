...
mergers = ['1_1645', '1_9618']
possible_mergers = ['1_9772', '1_1626', '2_3729', '1_5811', '1_4456', '2_19468', '2_3273', '2_1139', '1_12336', '2_13471']
"""
...
all_merger_subsets = []
for r in range(len(possible_mergers) + 1):
    for subset in itertools.combinations(possible_mergers, r):
        all_merger_subsets.append(list(subset))
total_iterations = (len(classification_thresholds) * len(debiaseds) * len(bin_sizes) * len(phys_seps) * len(min_phys_seps) * len(min_lxs) * len(sfr_thresholds) * len(all_merger_subsets))
                ...
                for min_phys_sep in min_phys_seps:
                    for min_lx in min_lxs:
                        for sfr_threshold in sfr_thresholds:
                            for merger_subset in all_merger_subsets:
                                i += 1
                                ...
                                bcg_df = bcg_df[(bcg_df['GAL_sdss_i_modSX_C2_PA'] > 0) & (~bcg_df['CLUS_ID'].isin(merger_subset)) & (~bcg_df['CLUS_ID'].isin(mergers))]
                                ...
                                if abs(popt_ef_frac[0]) > abs(best_ef_amplitude):
                                    best_ef_params = {"Classification": classification_threshold, "Debiased": debiased, "Bin Size": bin_size, "Max Physical Separation": phys_sep, "Min Physical Separation": min_phys_sep, "Max Velocity": max_vel, "Min Velocity": min_vel, "Min Lx": min_lx, "Number of satellites in sample": len(sat_majoraxis_list), "SFR Threshold": sfr_threshold, "Mergers": (merger_subset or []) + (mergers or [])}
                                    best_ef_amplitude = popt_ef_frac[0]
                                if abs(popt_sq_frac[0]) > abs(best_sq_amplitude):
                                    best_sq_params = {"Classification": classification_threshold, "Debiased": debiased, "Bin Size": bin_size, "Max Physical Separation": phys_sep, "Min Physical Separation": min_phys_sep, "Max Velocity": max_vel, "Min Velocity": min_vel, "Min Lx": min_lx, "Number of satellites in sample": len(sat_majoraxis_list), "SFR Threshold": sfr_threshold, "Mergers": (merger_subset or []) + (mergers or [])}
...
with open("mergers_results.json", "w") as f:
