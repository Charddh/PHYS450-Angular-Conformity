...
debiaseds = [0, 1]
bin_sizes = [30]
phys_seps = [250, 500, 750, 1000, 1250, 1500, 1750, 2000, 2250, 2500, 2750, 3000, 3250, 3500]
max_vels = [1500, 2000, 2500, 3000, 3500, 4000, 4500, 4750, 5000, 5250, 5500, 5750, 6000, 6500, 7000, 7500, 8000]
min_satellite_masses = [10.18, 10.27]
            ...
            for phys_sep in phys_seps:
                for max_vel in max_vels:
                    for min_satellite_mass in min_satellite_masses:
                        ...
                        print("popt", popt_ef_frac[0], popt_sq_frac[0])
                                                    
                        if abs(popt_ef_frac[0]) > abs(best_ef_amplitude):
                            best_ef_params = {"Classification": classification_threshold, "Debiased": debiased, "Bin Size": bin_size, "Physical Separation": phys_sep, "Max Velocity": max_vel, "Min Satellite Mass": min_satellite_mass}
                            best_ef_amplitude = popt_ef_frac[0]
                        if abs(popt_sq_frac[0]) > abs(best_sq_amplitude):
                            best_sq_params = {"Classification": classification_threshold, "Debiased": debiased, "Bin Size": bin_size, "Physical Separation": phys_sep, "Max Velocity": max_vel, "Min Satellite Mass": min_satellite_mass}
                            best_sq_amplitude = popt_sq_frac[0]
                        if abs((popt_ef_frac[0]) / np.sqrt(pcov_ef_frac[0,0])) > abs(best_ef_sigma):
                            best_ef_sigma_params = {"Classification": classification_threshold, "Debiased": debiased, "Bin Size": bin_size, "Physical Separation": phys_sep, "Max Velocity": max_vel, "Min Satellite Mass": min_satellite_mass}
                            best_ef_sigma = popt_ef_frac[0] / np.sqrt(pcov_ef_frac[0,0])
                        if abs((popt_sq_frac[0]) / np.sqrt(pcov_sq_frac[0,0])) > abs(best_sq_sigma):
                            best_sq_sigma_params = {"Classification": classification_threshold, "Debiased": debiased, "Bin Size": bin_size, "Physical Separation": phys_sep, "Max Velocity": max_vel, "Min Satellite Mass": min_satellite_mass}
...
print("Best sigma for sq fraction:", best_ef_sigma)
