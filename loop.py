...
min_satellite_mass = 10.5 #Minimum satellite galaxy mass.
...
classification_thresholds = [0.6, 0.8, 1]
debiaseds = [1]
bin_sizes = [30, 45]
phys_seps = [500, 750, 1000, 1250, 1500, 2000, 2500, 3000, 3500, 4000]
min_phys_seps = [0, 100, 250, 500]
max_vels = [3000, 3500, 4000, 4500, 5000, 6000, 7000, 8000, 9000]
min_vels = [0, 750, 1500, 2000, 2500]
min_satellite_masses = [10.18]
        ...
        for bin_size in bin_sizes:
            for phys_sep in phys_seps:
                for max_vel in max_vels:
                    for min_satellite_mass in min_satellite_masses:
                        for min_phys_sep in min_phys_seps:
                            for min_vel in min_vels:                
                                ...
                                if abs(popt_frac[0]) > abs(best_amplitude):
                                    best_params = {"Classification": classification_threshold, "Debiased": debiased, "Bin Size": bin_size, "Max Physical Separation": phys_sep, "Min Physical Separation": min_phys_sep, "Max Velocity": max_vel, "Min Velocity": min_vel, "Min Satellite Mass": min_satellite_mass}
                                    best_amplitude = popt_frac[0]
                                if abs(popt_sfr_frac[0]) > abs(best_sfr_amplitude):
                                    best_sfr_params = {"Classification": classification_threshold, "Debiased": debiased, "Bin Size": bin_size, "Max Physical Separation": phys_sep, "Min Physical Separation": min_phys_sep, "Max Velocity": max_vel, "Min Velocity": min_vel, "Min Satellite Mass": min_satellite_mass}
                                    best_sfr_amplitude = popt_sfr_frac[0]
                                if abs((popt_frac[0]) / np.sqrt(pcov_frac[0,0])) > abs(best_sigma):
                                    best_sigma_params = {"Classification": classification_threshold, "Debiased": debiased, "Bin Size": bin_size, "Max Physical Separation": phys_sep, "Min Physical Separation": min_phys_sep, "Max Velocity": max_vel, "Min Velocity": min_vel, "Min Satellite Mass": min_satellite_mass}
                                    best_sigma = popt_frac[0] / np.sqrt(pcov_frac[0,0])
                                if abs((popt_sfr_frac[0]) / np.sqrt(pcov_sfr_frac[0,0])) > abs(best_sfr_sigma):
                                    best_sfr_sigma_params = {"Classification": classification_threshold, "Debiased": debiased, "Bin Size": bin_size, "Max Physical Separation": phys_sep, "Min Physical Separation": min_phys_sep, "Max Velocity": max_vel, "Min Velocity": min_vel, "Min Satellite Mass": min_satellite_mass}
                                    best_sfr_sigma = popt_sfr_frac[0] / np.sqrt(pcov_sfr_frac[0,0])
                                
                                if abs(popt_ef_frac[0]) > abs(best_ef_amplitude):
                                    best_ef_params = {"Classification": classification_threshold, "Debiased": debiased, "Bin Size": bin_size, "Max Physical Separation": phys_sep, "Min Physical Separation": min_phys_sep, "Max Velocity": max_vel, "Min Velocity": min_vel, "Min Satellite Mass": min_satellite_mass}
                                    best_ef_amplitude = popt_ef_frac[0]
                                if abs(popt_sq_frac[0]) > abs(best_sq_amplitude):
                                    best_sq_params = {"Classification": classification_threshold, "Debiased": debiased, "Bin Size": bin_size, "Max Physical Separation": phys_sep, "Min Physical Separation": min_phys_sep, "Max Velocity": max_vel, "Min Velocity": min_vel, "Min Satellite Mass": min_satellite_mass}
                                    best_sq_amplitude = popt_sq_frac[0]
                                if abs((popt_ef_frac[0]) / np.sqrt(pcov_ef_frac[0,0])) > abs(best_ef_sigma):
                                    best_ef_sigma_params = {"Classification": classification_threshold, "Debiased": debiased, "Bin Size": bin_size, "Max Physical Separation": phys_sep, "Min Physical Separation": min_phys_sep, "Max Velocity": max_vel, "Min Velocity": min_vel, "Min Satellite Mass": min_satellite_mass}
                                    best_ef_sigma = popt_ef_frac[0] / np.sqrt(pcov_ef_frac[0,0])
                                if abs((popt_sq_frac[0]) / np.sqrt(pcov_sq_frac[0,0])) > abs(best_sq_sigma):
                                    best_sq_sigma_params = {"Classification": classification_threshold, "Debiased": debiased, "Bin Size": bin_size, "Max Physical Separation": phys_sep, "Min Physical Separation": min_phys_sep, "Max Velocity": max_vel, "Min Velocity": min_vel, "Min Satellite Mass": min_satellite_mass}
                                    best_sq_sigma = popt_sq_frac[0] / np.sqrt(pcov_sq_frac[0,0])

print("Best parameters for ef fraction:", best_ef_params)
...
print("Best parameters for sq fraction:", best_sq_params)
