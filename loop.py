...
debiaseds = [0, 1]
bin_sizes = [45]
phys_seps = [500, 750, 1000, 1250, 1500, 1600, 1750, 1900, 2000, 2100, 2250, 2500, 2750, 3000, 3250, 3500]
max_vels = [3000, 3500, 4000, 4500, 4750, 5000, 5250, 5400, 5500, 5750, 6000, 6500, 7000, 7500, 8000]
min_satellite_masses = [10.0, 10.2, 10.5]
                ...
                for max_vel in max_vels:
                    for min_satellite_mass in min_satellite_masses:                  
                        ...
                        if abs(popt_frac[0]) > abs(best_amplitude):
                            best_params = {"Classification": classification_threshold, "Debiased": debiased, "Bin Size": bin_size, "Physical Separation": phys_sep, "Max Velocity": max_vel, "Min Satellite Mass": min_satellite_mass}
                            best_amplitude = popt_frac[0]
                        if abs(popt_sfr_frac[0]) > abs(best_sfr_amplitude):
                            best_sfr_params = {"Classification": classification_threshold, "Debiased": debiased, "Bin Size": bin_size, "Physical Separation": phys_sep, "Max Velocity": max_vel, "Min Satellite Mass": min_satellite_mass}
                            best_sfr_amplitude = popt_sfr_frac[0]
                        if abs((popt_frac[0]) / np.sqrt(pcov_frac[0,0])) > abs(best_sigma):
                            best_sigma_params = {"Classification": classification_threshold, "Debiased": debiased, "Bin Size": bin_size, "Physical Separation": phys_sep, "Max Velocity": max_vel, "Min Satellite Mass": min_satellite_mass}
                            best_sigma = popt_frac[0] / np.sqrt(pcov_frac[0,0])
                        if abs((popt_sfr_frac[0]) / np.sqrt(pcov_sfr_frac[0,0])) > abs(best_sfr_sigma):
                            best_sfr_sigma_params = {"Classification": classification_threshold, "Debiased": debiased, "Bin Size": bin_size, "Physical Separation": phys_sep, "Max Velocity": max_vel, "Min Satellite Mass": min_satellite_mass}
